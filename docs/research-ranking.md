...
| 3 | Multi-model plus selective multi-prompt aggregation in log-odds space | High | ForecastBench's top-three-model aggregate compared aggregation methods; geometric mean/log-odds methods scored 0.194 vs 0.197 for median/trimmed mean. Silicon-crowd Study 1 used 12 LLMs on 31 live Metaculus questions and was statistically indistinguishable from 925-human crowd. Prompt-only studies are weaker, so model diversity gets priority over variant-only diversity. | OpenAI `gpt-5.5`, xAI `grok-4.20-multi-agent-0309`, and Claude `claude-opus-4-6` are combined by weighted log-odds mean. Defaults use one paid-provider variant and four cheap Grok variants; full prompt ensembling is configurable. |
