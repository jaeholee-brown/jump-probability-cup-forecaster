...
|---:|---|---|---|---|
| 1 | Fresh retrieval plus relevance filtering and summarization | Very high | Halawi et al. built a retrieval-augmented forecasting system over 914 test questions from a 5,516-question curated set. Best raw GPT-4 baseline Brier was 0.208; full system reached 0.179. Removing both retrieval and fine-tuning fell to 0.206. | Four Grok multi-agent web/X-search evidence passes per match when available: stable overview, explicit base rates, late-news/lineups, and market-specific micro evidence; OpenAI web search as fallback; optional odds data; optional Firecrawl search+scrape snippets as source context; compact merged evidence passed into the forecaster. |
