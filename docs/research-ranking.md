...
|---:|---|---|---|---|
| 1 | Fresh retrieval plus relevance filtering and summarization | Very high | Halawi et al. built a retrieval-augmented forecasting system over 914 test questions from a 5,516-question curated set. Best raw GPT-4 baseline Brier was 0.208; full system reached 0.179. Removing both retrieval and fine-tuning fell to 0.206. | Grok multi-agent web/X-search evidence pass per match when available; OpenAI web search as fallback; optional odds data; compact evidence passed into the forecaster. |
...
| 3 | Multi-model plus multi-prompt aggregation in log-odds space | High | ForecastBench's top-three-model aggregate compared aggregation methods; geometric mean/log-odds methods scored 0.194 vs 0.197 for median/trimmed mean. Silicon-crowd Study 1 used 12 LLMs on 31 live Metaculus questions and was statistically indistinguishable from 925-human crowd. Prompt-only studies are weaker, so model diversity gets priority over variant-only diversity. | Each configured provider runs the four prompt variants; OpenAI `gpt-5.5`, xAI `grok-4.20-multi-agent-0309`, and Claude `claude-opus-4-6` are combined by weighted log-odds mean, with configurable extremization. |
...
| 5 | Balanced scratchpad: rephrase, yes/no reasons, calibration check | Medium | Halawi and ForecastBench use scratchpad prompts in strong baselines, and ForecastBench's top baseline models often use scratchpad. But the prompt-engineering paper finds prompt-only effects are mostly negligible. | One concise scratchpad variant; final output is structured JSON, not free-form reasoning. |
...
The bot therefore runs separate prompt variants across separate model families and combines probabilities in log-odds space. It applies configurable mild extremization (`EXTREMIZE_ALPHA=1.05`) and shrinkage when evidence is weak.
